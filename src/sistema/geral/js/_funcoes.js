...
function montaEntregas(){
	$.ajax({async: true,
			...
			dataType: "html",
			url: "planodeviagem/adicionaPlanoDeViagem.php",
			data: "id_ordemDeEmbarque="+$("#txt_id_ordemDeEmbarque").attr("value")+"&dataInicio="+$("#txt_dataInicio").attr("value")+"&horaInicio="+$("#txt_horaInicio").attr("value")+"&circuito="+$("#cmb_circuito").attr("value")+"&tipoViagem="+$("#cmb_tipoViagem").attr("value")+"&dataSaidaDeposito="+$("#txt_dataSaida").attr("value")+"&montagemManual="+$("#chk_manual").is(":checked")+"&montaEntregas=1",
			beforeSend: mostrarCarregando,
			success: function(dados){
				aposCarregamento();
