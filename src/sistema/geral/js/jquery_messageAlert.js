...
(function($){
	$.fn.MsgAlert = function(options) {
		...
		return this.each(function() { 
			...
			if (alertContent == "") { alertContent = options.alertContentDefault; }
			...
			if (alertTitle == "") { alertTitle = options.alertTitleDefault; }	
			...
			$(newMsgAlert).dialog({ 
				...
				title: alertTitle,
				close: function(event, ui) { closeThisAlert(event, ui, $(newMsgAlert)); } ,
				minHeight: 0,
			...
			if(options.wait) {				
				$("#MsgAlert-" + alertNumber).oneTime((options.intervalTime * alertNumber), function() { showThisAlert($(this), alertBottomSpace) });		
			}
			else {
			...
			function showThisAlert(myMsgAlert, alertBottom) {
				 ...
				 $(myMsgAlert).parent().css({position: options.cssPosition, bottom: alertBottom + "px",right: options.cssRight + "px",top:"auto",left:"auto"}); 
				 $(myMsgAlert).fadeIn("slow"); 
				 if (options.autoClose) {
					$(myMsgAlert).oneTime(options.closeTime, function() { $(myMsgAlert).dialog('close' ); });
				 }
			...
			function closeThisAlert(event, ui, myMsgAlert) {
				...
				for (i=totalAlerts; i>0; i--) {
					if (i == 1) {
						...
						$("#MsgAlert-1").parent().animate({ bottom:  options.initialSpaceHeight + "px"}, 500 );
					}
					else {
						var alertBottomSpace = options.initialSpaceHeight; 
						for (a=1; a < i; a++) {
						...
						$("#MsgAlert-" + i).parent().animate({ bottom:  alertBottomSpace + "px"}, 500 );
					}
				}
			}
